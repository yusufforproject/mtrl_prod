  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (data.containsKey('data')) {
        ...
        if (Variable.schedules.any((element) => element.containsValue(null))) {
          Variable.schedules = [];
          return false;
        }
...
Future<bool> getSuratJalan() async {
  var tanggal = Variable.pickedDate.isEmpty ? Variable.dateSys : Variable.pickedDate;
  var shift = Variable.shift.isEmpty ? Variable.shfSys : Variable.shift;
  // var shift = Variable.shift.isEmpty ? Variable.shfSys : Variable.shift;
  try {
    final response = await http.get(
      Uri.parse('${Variable.baseUrl}/apis/get_suratjalan.php?tanggal=$tanggal&shift=$shift&mc=${Variable.mcnSelected}&opr=${Variable.oprCode}'),
      // body: {
      //   'sec': '${Variable.sect}${Variable.subSec}',
      // },
    );
    ...
    if (response.statusCode == 200) {
      ...
      if (data.containsKey('data')) {
        ...
        if (Variable.suratJalan.any((element) => element.containsValue(null))) {
          Variable.suratJalan = [];
          return false;
        }
