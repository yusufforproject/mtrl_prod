        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: SingleChildScrollView(
              child: ListBody(
                children: [
                  Text('${'BOM (' + Variable.schedules[0]['size']}):', style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
                  ...Variable.bom.map((item) {
                    return Text('- ' + item['child'], style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold),);
                  }).toList(),
                ],
              ),
            ),
              ...
              Row(
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      autofocus: true,
                      controller: Variable.treatmentDetails.isEmpty
                          ? _controller
                          : TextEditingController(
                              text: Variable.treatmentDetails[0]['bc_entried']),
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    return Text('Error: ${snapshot.error}');
                      ...
                      Variable.treatmentDetails.isEmpty) {
                        ...
                        ..addAll(Variable.treatmentDetails.map((detail) {
                          return TableRow(children: [
                            ...
                            TableCell(
                              child: Padding(
                                padding: const EdgeInsets.all(8.0),
                                child: Text('${detail['akt']} MTR', style: TextStyle(color: double.parse(detail['akt']) <= 0 ? Colors.white : Colors.black, backgroundColor: double.parse(detail['akt']) <= 0 ? Colors.red[600] : Colors.transparent),),
                              ),
                            ),
                          ]);
                        }).toList())
